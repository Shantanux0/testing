...
public class TestPortalService {
    ...
    public TestResultResponse submitTest(String email, SubmitTestRequest request) {
        ...
        int score = calculateScore(request.getAnswers(), questions, correctAnswers);
    ...
    private int calculateScore(List<UserAnswer> userAnswers, List<TestQuestion> questions, List<String> correctAnswers) {
        int score = 0;
        ...
        for (int i = 0; i < correctAnswers.size(); i++) {
            ...
            List<String> options = (i < questions.size()) ? questions.get(i).getOptions() : Collections.emptyList();
            if (isAnswerCorrect(userAnswer, correctAnswers.get(i), options)) {
                score++;
            }
        }
        return score;
    ...
    private TestResultResponse buildTestResult(UserSkillTestEntity test, List<TestQuestion> questions,
        ...
        for (int i = 0; i < questions.size(); i++) {
            ...
            boolean isCorrect = isAnswerCorrect(userAnswer, correctAnswer, q.getOptions());

            questionResults.add(QuestionResult.builder()
                    .questionNumber(q.getQuestionNumber())
                    .question(q.getQuestion())
                    .correctAnswer(getCorrectAnswerDisplayText(correctAnswer, q.getOptions()))
    ...
    private boolean isAnswerCorrect(String userAnswer, String correctAnswer, List<String> options) {
        ...
        // If options are available and correct answer is a single letter (A-D)
        if (options != null && !options.isEmpty() && correctAnswer.length() == 1) {
            int correctIdx = Character.toUpperCase(correctAnswer.charAt(0)) - 'A';
            if (correctIdx >= 0 && correctIdx < options.size()) {
                String correctOptionText = options.get(correctIdx).trim();
                if (userAnswer.equalsIgnoreCase(correctOptionText)) {
                    return true;
                }
                // Also check if correctOptionText starts with "A. OptionText" and user answer matches the option text or letter
                if (correctOptionText.length() > 2) {
                    char firstChar = Character.toUpperCase(correctOptionText.charAt(0));
                    char correctChar = Character.toUpperCase(correctAnswer.charAt(0));
                    char delimiter = correctOptionText.charAt(1);
                    if (firstChar == correctChar && (delimiter == '.' || delimiter == ')' || delimiter == ' ')) {
                        String strippedOptionText = correctOptionText.substring(2).trim();
                        if (userAnswer.equalsIgnoreCase(strippedOptionText)) {
                            return true;
                        }
                    }
                }
    ...
    private String getCorrectAnswerDisplayText(String correctAnswer, List<String> options) {
        if (correctAnswer == null) return "";
        correctAnswer = correctAnswer.trim();
        if (options != null && !options.isEmpty() && correctAnswer.length() == 1) {
            int correctIdx = Character.toUpperCase(correctAnswer.charAt(0)) - 'A';
            if (correctIdx >= 0 && correctIdx < options.size()) {
                return options.get(correctIdx);
            }
        }
